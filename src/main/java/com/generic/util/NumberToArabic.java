...
public class NumberToArabic {
	...
	public static enum Currency {
		AED, SYP, SAR, TND, XAU, JOD, BHD, USD, EUR
	}
	...
	private static String[] englishTens = new String[] { "Twenty", "Thirty", "Forty", "Fifty", "Sixty", "Seventy",
			"Eighty", "Ninety" };
	...
	private static String[] arabicOnes = new String[] { "", "واحد", "اثنان", "ثلاثة", "أربعة", "خمسة", "ستة", "سبعة",
			"ثمانية", "تسعة", "عشرة", "أحد عشر", "اثنا عشر", "ثلاثة عشر", "أربعة عشر", "خمسة عشر", "ستة عشر",
			"سبعة عشر", "ثمانية عشر", "تسعة عشر" };

	private static String[] arabicFeminineOnes = new String[] { "", "إحدى", "اثنتان", "ثلاث", "أربع", "خمس", "ست",
			"سبع", "ثمان", "تسع", "عشر", "إحدى عشرة", "اثنتا عشرة", "ثلاث عشرة", "أربع عشرة", "خمس عشرة", "ست عشرة",
			"سبع عشرة", "ثماني عشرة", "تسع عشرة" };

	private static String[] arabicTens = new String[] { "عشرون", "ثلاثون", "أربعون", "خمسون", "ستون", "سبعون", "ثمانون",
			"تسعون" };

	private static String[] arabicHundreds = new String[] { "", "مائة", "مئتان", "ثلاثمائة", "أربعمائة", "خمسمائة",
			"ستمائة", "سبعمائة", "ثمانمائة", "تسعمائة" };

	private static String[] arabicAppendedTwos = new String[] { "مئتا", "ألفا", "مليونا", "مليارا", "تريليونا",
			"كوادريليونا", "كوينتليونا", "سكستيليونا" };

	private static String[] arabicTwos = new String[] { "مئتان", "ألفان", "مليونان", "ملياران", "تريليونان",
			"كوادريليونان", "كوينتليونان", "سكستيليونان" };

	private static String[] arabicGroup = new String[] { "مائة", "ألف", "مليون", "مليار", "تريليون", "كوادريليون",
			"كوينتليون", "سكستيليون" };

	private static String[] arabicAppendedGroup = new String[] { "", "ألفاً", "مليوناً", "ملياراً", "تريليوناً",
			"كوادريليوناً", "كوينتليوناً", "سكستيليوناً" };

	private static String[] arabicPluralGroups = new String[] { "", "آلاف", "ملايين", "مليارات", "تريليونات",
			"كوادريليونات", "كوينتليونات", "سكستيليونات" };
	...
	public NumberToArabic(BigDecimal number, Currency currency, String englishPrefixText, String englishSuffixText,
			String arabicPrefixText, String arabicSuffixText) {
		NumberToArabic.number = number;
		NumberToArabic.currency = currency;
		NumberToArabic.currencyInfo = new CurrencyInfo(currency);
	...
	private static void numberToArabic(BigDecimal number, CurrencyInfo currency, String englishPrefixText,
	...
	public static String convertToEnglish() {
		...
		formattedNumber += (retVal != "")
				? (_intergerValue == 1 ? currencyInfo.englishCurrencyName : currencyInfo.englishPluralCurrencyName)
				: "";
		formattedNumber += (decimalString != "") ? " and " : "";
		formattedNumber += (decimalString != "") ? decimalString : "";
		formattedNumber += (decimalString != "") ? " " + (_decimalValue == 1 ? currencyInfo.englishCurrencyPartName
	...
	private static String processArabicGroup(int groupNumber, int groupLevel, BigDecimal remainingNumber) {
		...
		if (tens > 0) {
			if (tens < 20) { // if we are processing under 20 numbers
				if (tens == 2 && hundreds == 0 && groupLevel > 0) { // This is special case for number 2 when it comes
																	// alone in the group
	...
	public static String convertToArabic() {
				...
				if (group > 0) {
					...
					if (numberToProcess != 2) {
						if (numberToProcess % 100 != 1) {
							if (numberToProcess >= 3 && numberToProcess <= 10) // for numbers between 3 and 9 we use
																				// plural name
		...
		formattedNumber += (retVal != "") ? retVal : "";
		if (_intergerValue != 0) { // here we add currency name depending on _intergerValue : 1 ,2 , 3--->10 ,
									// 11--->99
		...
		if (_decimalValue != 0) { // here we add currency part name depending on _intergerValue : 1 ,2 , 3--->10 ,
									// 11--->99
			formattedNumber += " ";
	...
	class CurrencyInfo {
		...
		public CurrencyInfo(Currency currency) {
			...
			case EUR:
				...
				arabic2CurrencyName = "يوروان";
				arabic310CurrencyName = "يوروات";
				arabic1199CurrencyName = "يورواً";
				arabic1CurrencyPartName = "سنت";
				arabic2CurrencyPartName = "سنتان";
				arabic310CurrencyPartName = "سنتات";
				arabic1199CurrencyPartName = "سنتاً";
