...
import com.smat.ins.model.data.lazyloading.LazyCorrespondenceNoteDataModel;
...
public class OutBoxBean implements Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 8863296966187973024L;
	// #region"properties"
	// entity object
	private Long correspondenceID;
	private String correspondenceStr;

	private Long correspondenceNoteID;
	private String correspondenceNoteStr;
	...
	// LazyData Model
	private LazyDataModel<CorrespondenceNote> correspondenceNoteLazyDataModel;
	...
	public LazyDataModel<CorrespondenceNote> getCorrespondenceNoteLazyDataModel() {
		return correspondenceNoteLazyDataModel;
	}

	public void setCorrespondenceNoteLazyDataModel(LazyDataModel<CorrespondenceNote> correspondenceNoteLazyDataModel) {
		this.correspondenceNoteLazyDataModel = correspondenceNoteLazyDataModel;
	}
	...
	public Long getCorrespondenceNoteID() {
		return correspondenceNoteID;
	}

	public void setCorrespondenceNoteID(Long correspondenceNoteID) {
		this.correspondenceNoteID = correspondenceNoteID;
	}

	public String getCorrespondenceNoteStr() {
		return correspondenceNoteStr;
	}

	public void setCorrespondenceNoteStr(String correspondenceNoteStr) {
		this.correspondenceNoteStr = correspondenceNoteStr;
	}
  ...
  // #endregion
	...
	public void init() {
		...
		correspondenceStr = UtilityHelper.getRequestParameter("ob");
		correspondenceNoteStr = UtilityHelper.getRequestParameter("obn");
		...
		try {
			if (correspondenceStr != null) {
				...
				for(CorrespondenceArchive correspondenceArchive:correspondenceArchives) {
					List<ArchiveDocumentFile> archiveDocFiles=archiveDocumentFileService.getBy(correspondenceArchive.getArchiveDocument());
			...
			if (correspondenceNoteStr != null) {
				correspondenceNoteID = Long.valueOf(UtilityHelper.decipher(correspondenceNoteStr));
				correspondenceNote = correspondenceNoteService
						.findById(correspondenceNoteID);
				correspondenceRecipient = getBy(correspondenceNote);
				
				noteMsg = new String(Base64.getMimeDecoder()
						.decode(correspondenceNote.getNote()));
				correspondenceNotes=correspondenceNoteService.getCorrespondenceNotes(correspondence);
				userAliasRecipientList = userAliasService.getListRecipients(correspondenceNote.getUserAlias());
			}
			viewDocFiles = false;
	...
	public OutBoxBean() throws Exception {
		toArchive=false;
		...
		correspondenceArchiveService=(CorrespondenceArchiveService) BeanUtility.getBean("correspondenceArchiveService");
		...
		cabinetFolderDocumentService=(CabinetFolderDocumentService) BeanUtility.getBean("cabinetFolderDocumentService");
		...
		if (myUserAliasList != null || !myUserAliasList.isEmpty()) {
			...
			correspondenceNoteLazyDataModel = new LazyCorrespondenceNoteDataModel(sysUserLogin,
					correspondenceState);
	...
	public void assignCorrespondence(CorrespondenceNote correspondenceNote) throws Exception {
		correspondenceID = correspondenceNote.getCorrespondence().getId();
		correspondenceStr = UtilityHelper.cipher(correspondenceID.toString());

		correspondenceNoteID = correspondenceNote.getId();
		correspondenceNoteStr = UtilityHelper.cipher(correspondenceNoteID.toString());
	...
	public boolean doValidate() {
	 ...
	 public String doComment() {
		 try {
		    ...
		    recipientNote
					.setHtmlNote(Base64.getMimeEncoder().encode(comment.getBytes()));
		    recipientNote
					.setNote(Base64.getMimeEncoder().encode(comment.getBytes()));
		    ...
		    correspondenceTransmissionRecpient.setUserAliasByFromAlias(correspondenceRecipient.getCorrespondenceNote().getUserAlias());
			...
			for (CorrespondenceRecipient recipient:recipientList) {
				Long inboxNewCount=correspondenceRecipientService.getInboxCountBySysUser(recipient.getUserAlias().getSysUserBySysUser(), correspondenceState);
	 ...
	 public CorrespondenceRecipient getBy(CorrespondenceNote note) {
		 if(note.getCorrespondenceRecipients().isEmpty()) {
			 ...
			 CorrespondenceRecipient correspondenceRecipient=	(CorrespondenceRecipient) note.getCorrespondenceRecipients().iterator().next();
			 return correspondenceRecipient;
		}
	 }
	 
	 public void viewInbox(SysUser sysUser) {
		 correspondenceRecipientLazyDataModel = new LazyCorrespondenceRecipientDataModel(sysUser,
					correspondenceState);
		 allCorrespondenceRecipientLazyDataModel=new LazyAllCorrespondenceRecipientDataModel(sysUser,
					correspondenceState);
	 }
	 
	 public String iconRecipientStatus(CorrespondenceRecipient correspondenceRecipient) {
		if(correspondenceRecipient.getViewedDate()==null)
			return "pi pi-minus";
		else
			return "fas fa-check-double";
	 }
	 
	 public String iconColorRecipientStatus(CorrespondenceRecipient correspondenceRecipient) {
			if (correspondenceRecipient.getViewedDate() == null)
				return "color:#000;";
			else if (correspondenceRecipient.getProcessingDate() == null)
				return "font-size:24px;color:#2196F3;";
			else
				return "font-size:24px;color:#0ACC00;";
	 }
	...
	//#endregion
		...
		public void openNewDocument() throws Exception {
			...
			Long maxArchiveDocumentCodeLenght = archiveDocumentService.getMaxArchiveDocumentCode(null, this.cabinetFolder.getId());
		...
		private void intializeRootAndFirstLevel() throws Exception {
			...
			for (ArchiveDocument rootArchiveDocument : archiveDocuments) {
				...
				List<ArchiveDocument> firstLevel = archiveDocumentService.getByParent(rootArchiveDocument, cabinetFolder.getId(),
						null, null);
		...
		public void onNodeArchSelect(NodeSelectEvent event) throws Exception {
			if (selectedNode != null) {
				selectedArchiveDocument = archiveDocumentService
						.getArchiveDocumentWithDetails((ArchiveDocument) selectedNode.getData(), this.cabinetFolder.getId());
				breadcrumbList = getListBreadcrumbs(selectedArchiveDocument, this.cabinetFolder.getId());
				archiveDocumentChildrens = archiveDocumentService.getByParent(selectedArchiveDocument, this.cabinetFolder.getId(),
						null, null);
		...
		public void getArchiveDocChildren(Long archiveDocumentId) throws Exception {
			...
			archiveDocumentChildrens = archiveDocumentService.getByParent(selectedArchiveDocument, cabinetFolder.getId(), null,
					null);
			if (!selectedArchiveDocument.getIsDirectory()) {
				archiveDocumentFiles = archiveDocumentFileService.getBy(selectedArchiveDocument);
				viewDocFiles = true;
		...
		public void addArchiveDoc() throws Exception {
			if (selectedArchiveDocument != null) {
				...
				if (archiveDocumentTypes != null && !archiveDocumentTypes.isEmpty())
					this.archiveDocument.setArchiveDocumentType(archiveDocumentTypes.get(0));
				this.archiveDocument.setYear((short) Calendar.getInstance().get(Calendar.YEAR));
				Long maxArchiveDocumentCodeLenght = archiveDocumentService.getMaxArchiveDocumentCode(null, cabinetFolder.getId());
				if (maxArchiveDocumentCodeLenght != null) {
					archiveDocument.setSeq(maxArchiveDocumentCodeLenght + 1);
					archiveDocument
							.setCode(String.format("%0" + documentCodeLength + "d", maxArchiveDocumentCodeLenght + 1));
			...
			try {
				if (event.getFile().getFileName().getBytes(java.nio.charset.StandardCharsets.UTF_8).length > 100) {
					UtilityHelper.addWarnMessage(
							localizationService.getInterfaceLabel().getString("fileNameMaxLengthShouldBe") + ":  "
		...
		public boolean foundInUploader(ArchiveDocumentFile archiveDocumentInstance)
			...
			for (ArchiveDocumentFile archiveDocumentFile : archiveDocumentFiles) {
				if (archiveDocumentFile.getName().trim().equalsIgnoreCase(archiveDocumentInstance.getName().trim())
					...
					UtilityHelper
							.addWarnMessage(
									new String(localizationService.getInterfaceLabel().getString("theFile")
											.getBytes(Charset.defaultCharset()), "UTF-8")
											+ " ( "
											+ new String(archiveDocumentInstance
													.getName().trim().getBytes(Charset.defaultCharset()), "UTF-8")
											+ " ) "
											+ new String(localizationService.getInterfaceLabel()
													.getString("hadNotBeenAdded").getBytes(Charset.defaultCharset()),
													"UTF-8"));
					PrimeFaces.current().ajax().update("form:messages");
					result = true;
				}
			...
			try {
				for (ArchiveDocumentFile archiveDocumentFile : archiveDocumentFilesResult) {
					if (archiveDocumentFile.getId().equals(archiveDocumentFileId)) {
						...
						Element elementDrawer = new Element(
								new NetworkElement(cabinetDefinitionSearchResult.getDrawerName(),
										"images/element-drawer.png"),
								"23em", "2em");
						...
						Element elementCabinetFolder = new Element(new NetworkElement(
								cabinetFolderSearchResult.getName(), "images/elementCabinetFolder.png"), "41em", "2em");
						...
						Element elementArchiveDocument = new Element(
								new NetworkElement(archiveDocumentSearchResult.getName(),
										"images/elementArchiveDocument.png"),
								"59em", "2em");
		...
		public void doSearchInAttribute() {
			try {
				...
				List<ArchiveDocument> archiveDocumentResultList = archiveDocumentService.getBy(cabinetFolder.getId(),
						archiveDocument.getArabicName(), archiveDocument.getEnglishName(),
						archiveDocument.getDescription(), archiveDocument.getArchiveDocumentType(),
						archiveDocument.getOrganizationByDivan(), archiveDocument.getFromEntity(),
						archiveDocument.getSubject(), archiveDocument.getYear());
