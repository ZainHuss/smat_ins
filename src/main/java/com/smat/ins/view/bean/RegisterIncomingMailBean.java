...
@Named
@ViewScoped
public class RegisterIncomingMailBean implements Serializable {
	...
	public RegisterIncomingMailBean() throws Exception {
	...
	public boolean doValidate() {
		...
		if (correspondence.getFromEntity() == null || correspondence.getFromEntity().trim().isEmpty()) {
			UtilityHelper.addErrorMessage(localizationService.getInterfaceLabel().getString("side") + "  "
					+ localizationService.getErrorMessage().getString("validateInput"));
			result = false;
		}

		if (correspondence.getFromNumber() == null || correspondence.getFromNumber().trim().isEmpty()) {
			UtilityHelper.addErrorMessage(localizationService.getInterfaceLabel().getString("theirNumber") + "  "
					+ localizationService.getErrorMessage().getString("validateInput"));
			result = false;
		}

		if (correspondence.getFromDate() == null) {
			UtilityHelper.addErrorMessage(localizationService.getInterfaceLabel().getString("theirDate") + "  "
					+ localizationService.getErrorMessage().getString("validateInput"));
			result = false;
		}
	...
	public String send() throws Exception {
		...
		if (doValidate()) {
			...
			if (correspondenceService.merge(correspondence)) {
				...
				registeredMailList = correspondenceService.getDraftBox(correspondenceState, boxType, sysUserLogin);
				str = "pretty:registerIncomingMail";
			}
		}
		return str;
	}
	...
	public String doSaveDraft() throws Exception {
		...
		if (correspondenceService.merge(correspondence)) {
			...
			registeredMailList = correspondenceService.getDraftBox(correspondenceState, boxType, sysUserLogin);
			return "pretty:registerIncomingMail";
		}
	...
	public void registerNewCorrespondence() throws Exception {
		...
		correspondence=correspondenceService.save(correspondence);
		if (correspondence != null) {		
			seq = correspondenceService.getLastSeq(boxType.getEnCode(), divan.getId());
			UtilityHelper.addInfoMessage(localizationService.getInfoMessage().getString("operationSuccess"));
			PrimeFaces.current().ajax().update("form:messages", "form:panelGrid_registeration");
		    PrimeFaces.current().executeScript("PF('widgetVar_title').jq.focus()");
		}
	...
	public void updateRecipientInfo() {	
		for (CorrespondenceRecipient correspondenceRecipient : recipientList) {
			if(correspondenceRecipient.getUserAlias().getAliasName().equals(recepientUser.getUserAlias().getAliasName())) {
				correspondenceRecipient.setPriorityType(priorityType);
				correspondenceRecipient.setPurposeType(purposeType);
				correspondenceRecipient.setNote(note);
				correspondenceRecipient.setHtmlNote(htmlNote);
	...
	// #region "linked mail"
		...
		public void search() throws Exception {
			correspondencesSearchList=correspondenceService.search(boxType, correspondenceSearch.getSeq(), correspondenceSearch.getCorrespondenceDate(), correspondenceSearch.getFromNumber(), correspondenceSearch.getFromEntity(), correspondenceSearch.getFromDate(), correspondenceSearch.getTitle());
		}
