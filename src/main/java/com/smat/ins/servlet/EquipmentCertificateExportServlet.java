...
public class EquipmentCertificateExportServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) 
        ...
        // return the stored PDF directly. Otherwise, if the request originated from a QR scan
        // we want to redirect users straight to the full certificate (no landing page). The QR
        // used by the inspection flow encodes `serialNo` and `reportNo` (or sticker), so treat
        // requests without explicit `format=html` as direct-download intent.
        ...
        // Default behavior: if the caller did not request explicit HTML (format=html), redirect
        // to the raw certificate endpoint so scanners go straight to the PDF.
        String format = request.getParameter("format");
        boolean explicitHtml = "html".equalsIgnoreCase(format);

        if (!explicitHtml) {
            // Redirect to the raw/download URL for convenience. Build a safe path.
            String ctx = request.getContextPath();
            String safePath = java.net.URLEncoder.encode((serialNo != null ? serialNo : "") + "&" + (stickerNo != null ? stickerNo : ""), "UTF-8");
            String rawUrl = ctx + request.getServletPath() + "/" + safePath + "?download=full";
            response.sendRedirect(rawUrl);
            return;
        }

        // If explicit HTML requested, keep the existing landing page behavior (for debugging/legacy)
        String ctx = request.getContextPath();
        String summaryUrl = ctx + "/api/equipment-cert/summary?serialNo=" + java.net.URLEncoder.encode(serialNo != null ? serialNo : "", "UTF-8") + "&stickerNo=" + java.net.URLEncoder.encode(stickerNo != null ? stickerNo : "", "UTF-8");
        String fullUrl = ctx + request.getServletPath() + "/" + java.net.URLEncoder.encode((serialNo != null ? serialNo : "") + "&" + (stickerNo != null ? stickerNo : ""), "UTF-8") + "?download=full";
        ...
        try (java.io.PrintWriter pw = response.getWriter()) {
            ...
            pw.println("</head><body><div class=\"wrap\"><div class=\"hdr\"><div><h2 style=\"margin:0;\">Inspection Certificate</h2><div class=\"muted\">Serial: " + (serialNo != null ? serialNo : "-") + " &amp; Sticker: " + (stickerNo != null ? stickerNo : "-") + "</div></div><div><a class=\"btn\" href=\"" + fullUrl + "\">Download Full Certificate</a></div></div>");
